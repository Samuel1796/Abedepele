...
const candidates = [
  { name: 'Kwame Nkrumah', position: 'President' },
  { name: 'Yaa Asantewaa', position: 'President' },
  { name: 'Kofi Annan', position: 'President' },
  { name: 'Efua Sutherland', position: 'Secretary' },
  { name: 'Abedi Pele', position: 'Financial Secretary' },
  { name: 'Ama Ata Aidoo', position: 'Financial Secretary' },
  { name: 'Samia Nkrumah', position: 'PRO' },
  { name: 'J.E.A. Mills', position: 'President' },
  { name: 'Asamoah Gyan', position: 'Financial Secretary' },
  { name: 'Kwame Ture', position: 'Secretary' },
  { name: 'Highest Yosh', position: 'PRO' }
];
    ...
    return (
      <div className="mb-8">
        <div className="bg-blue-500 text-white text-center py-2 mb-4">
          ...
          <p>Total Votes for {position}: {totalVotesForPosition}</p>
        </div>
        <div className="flex flex-wrap justify-center">
          {candidates
            ...
            .map(candidate => (
              loading ? (
                <div key={candidate.name} className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow m-2 p-4">
                ...
                <div key={candidate.name} className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow m-2 p-4">
                  <div className="flex flex-col items-center">
                    ...
                    <div className="mt-4">
                      <p className="text-lg font-bold text-blue-500">{votes[candidate.name] || 0} votes</p>
                      <p className="text-sm text-black ml-4">{totalVotesForPosition > 0 ? calculatePercentage(votes[candidate.name] || 0, totalVotesForPosition) : 0}%</p>
                    </div>
                  </div>
                </div>
              )
  ...
  return (
    <div className="container mx-auto p-4 min-h-screen" style={{ backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover' }}>
      <nav className="bg-blue-500 text-white py-4 px-6 flex justify-between items-center">
        <button onClick={() => navigate('/')} className="bg-white text-blue-500 px-4 py-2 rounded">Back</button>
        <h1 className="text-2xl font-bold">Live Votes Count</h1>
      </nav>
      <div className="text-center my-8">
        <h2 className="text-3xl font-bold text-blue-500">Total Votes Casted: {Object.values(votes).reduce((sum, voteCount) => sum + voteCount, 0)}</h2>
      </div>
